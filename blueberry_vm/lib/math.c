 ...
 * signature is (c, self, a, b) — b ignored for 1-arg ops */
#define BB_MATH_F1(name, cfn) \
static ci_ptr bb_math_##name(bb_coro_arg *c, ci_ptr self, ci_ptr a, ci_ptr b) { \
	(void)self; (void)b; BB_CHECK_NUMBER(a); \
...
static ci_ptr bb_math_##name(bb_coro_arg *c, ci_ptr self, ci_ptr a, ci_ptr b) { \
	(void)self; BB_CHECK_NUMBER(a); BB_CHECK_NUMBER(b); \
...
static ci_ptr bb_math_atan(bb_coro_arg *c, ci_ptr self, ci_ptr a, ci_ptr b) {
	(void)self; BB_CHECK_NUMBER(a);
...
static ci_ptr bb_math_sign(bb_coro_arg *c, ci_ptr self, ci_ptr a, ci_ptr b) {
	(void)self; (void)b; BB_CHECK_NUMBER(a);
...
static ci_ptr bb_math_deg(bb_coro_arg *c, ci_ptr self, ci_ptr a, ci_ptr b) {
	(void)self; (void)b; BB_CHECK_NUMBER(a);
	return (ci_ptr)ci_number_floating(ci_number_to_double(a) * (180.0 / M_PI));
}

static ci_ptr bb_math_rad(bb_coro_arg *c, ci_ptr self, ci_ptr a, ci_ptr b) {
	(void)self; (void)b; BB_CHECK_NUMBER(a);
...
static ci_ptr bb_math_isnan(bb_coro_arg *c, ci_ptr self, ci_ptr a, ci_ptr b) {
	(void)self; (void)b; BB_CHECK_NUMBER(a);
	return CI_BOOL(isnan(ci_number_to_double(a)));
}

static ci_ptr bb_math_isinf(bb_coro_arg *c, ci_ptr self, ci_ptr a, ci_ptr b) {
	(void)self; (void)b; BB_CHECK_NUMBER(a);
	return CI_BOOL(isinf(ci_number_to_double(a)));
}

static ci_ptr bb_math_isfinite(bb_coro_arg *c, ci_ptr self, ci_ptr a, ci_ptr b) {
	(void)self; (void)b; BB_CHECK_NUMBER(a);
	return CI_BOOL(isfinite(ci_number_to_double(a)));
}

/* math.clamp(x, lo, hi) */
static ci_ptr bb_math_clamp(bb_coro_arg *c, ci_ptr self, size_t nargs, ci_ptr *args) {
	(void)self;
	if (nargs < 3)
	...
	if (x < lo) x = lo;
	if (x > hi) x = hi;
	return (ci_ptr)ci_number_floating(x);
}

/* math.min(...) / math.max(...) — vararg */
static ci_ptr bb_math_min(bb_coro_arg *c, ci_ptr self, size_t nargs, ci_ptr *args) {
	(void)self;
	if (nargs < 1)
	...
	return (ci_ptr)ci_number_floating(best);
}

static ci_ptr bb_math_max(bb_coro_arg *c, ci_ptr self, size_t nargs, ci_ptr *args) {
	(void)self;
	if (nargs < 1)
...
static ci_ptr bb_math_type(bb_coro_arg *c, ci_ptr self, ci_ptr x, ci_ptr b) {
	(void)self; (void)b;
	if (CI_IS_INT(x))
...
static ci_ptr bb_math_boxed(bb_coro_arg *c, ci_ptr self, ci_ptr x, ci_ptr b) {
	(void)self; (void)b;
	if (CI_IS_NUMBER(x))
		return CI_BOOL(1);
	if (CI_IS_INT(x))
...
static ci_ptr bb_math_cmp(bb_coro_arg *c, ci_ptr self, size_t nargs, ci_ptr *args) {
	(void)self;
	if (nargs < 2)
...
static ci_ptr bb_math_eq(bb_coro_arg *c, ci_ptr self, size_t nargs, ci_ptr *args) {
	(void)self;
	if (nargs < 2)
	...
	return CI_BOOL(ci_number_cmp_eps(args[0], args[1], eps) == 0);
}

/* math.precision() — get; math.precision(eps) — set; returns current value */
static ci_ptr bb_math_precision(bb_coro_arg *c, ci_ptr self, ci_ptr x, ci_ptr b) {
	(void)self; (void)b;
