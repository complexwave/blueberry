...
static bb_var_ret bb_coro_resume_method(bb_coro *c, ci_ptr coro_ptr, size_t nargs, ci_ptr *args, size_t nrets) {
	BB_CHECK_CORO(coro_ptr);
	bb_coro *coro = (bb_coro *)coro_ptr;

	printf("coro:resume called with %zu args\n", nargs);

	if(coro->flags == BB_CORO_SUSPENDED){
		ci_ptr* yield_rets = coro->yield.args + coro->yield.nargs;
		
		size_t yield_nrets = coro->yield.nrets;
		
		while(yield_nrets && nargs){
			*(yield_rets++) = *(args++);
			printf("coro:resume called with %p args\n", *(args));
			yield_nrets--; nargs--;
		}
		
	} else 
	if(coro->flags == BB_CORO_NEW ){
		...
		bb_coro_error(c, "trying to resume coro(%p) unknown state %p", coro, coro->flags);
	}
	...
	if(coro->flags == BB_CORO_SUSPENDED){
		printf("coro:resume returned from yielded coro\n");
		
		rets_cnt = coro->yield.nargs;
		rets = coro->yield.args;
		
		printf("coro:resume  %d %p, requested %d\n", rets_cnt, rets, rets_cnt);
	...
	rets = coro->stack->data + rets_frame->stack_base;
	rets_cnt = CI_INT(rets[0]);
	rets++;

	copy_args2rets:;
	
	printf("coro:resume returned %d %p, requested %d\n", rets_cnt, rets[0], nrets);
	
	while(rets_cnt && nrets){
		args[nargs++] = *rets;
		rets_cnt--;
		nrets--;
		rets++;
...
// this is BB_FN_ADVANCED function - it consumes stack and return directly from bb_vm_execute
BB_PRESERVE_NONE void bb_coro_yield_fn(bb_coro *c, ci_ptr* base, size_t nargs, size_t nrets) {
	
	printf("coro.yield called with %p %p %p %p args\n", c, base, nargs, nrets);
...
/* coro.new(fn) — create new coroutine from function */
static ci_ptr bb_coro_new_fn(bb_coro_arg *c, ci_ptr_arg self, ci_ptr fn_arg) {
...
static void bb_lib_coro_init(bb_vm *vm) {
	...
	static const bb_cfunc coro_lib[] = {
		...
		{ "yield",  bb_coro_yield_fn,        BB_FN_ADVANCED },
	};
